...
module CandlestickServices
  class CreateDateService
    ...
    def check_last_date_is_previous_date date
    ...
    def get_binance_data merchandise_rate, start_time
      records = []
      period = (Time.zone.now.to_datetime - Time.at(start_time).to_datetime).to_i
      period_loop = 1000
      loop_number = period/period_loop
      (0..loop_number).each do |num|
        start_time = (Time.at(start_time).to_datetime + period_loop*num).to_datetime.to_i
    ...
    def build_candlestick_records merchandise_rate, b_data
      ...
      b_data.each do |record|
        ...
        next if check_last_date_is_previous_date record_date
        open = record[1].to_f
        high = record[2].to_f
        low = record[3].to_f
        close = record[4].to_f
